...
# log normal errors
def log_normal_errors(metrics, where_to_write, first_line, logger):
    print(first_line)
    ...
    #NEPTUNE AI LOGGER krishksk
    params = { 'mean':metrics['mean'], 'median':metrics['median'], 'rmse':metrics['rmse'],
        'a1':metrics['a1'], 'a2':metrics['a2'], 'a3':metrics['a3'], 'a4':metrics['a4'], 'a5':metrics['a5']}
    
    if logger is not None:
        logger['metrics/params'].append(params)
        logger['metrics/mean'].append(metrics['mean'])
        logger['metrics/median'].append(metrics['median'])
        logger['metrics/rmse'].append(metrics['rmse'])
        logger['metrics/a1'].append(metrics['a1'])
        logger['metrics/a2'].append(metrics['a2'])
        logger['metrics/a3'].append(metrics['a3'])
        logger['metrics/a4'].append(metrics['a4'])
        logger['metrics/a5'].append(metrics['a5'])
...
def visualize(args, img, gt_norm, gt_norm_mask, norm_out_list, total_iter):
    ...
    # input image
    target_path = '%s/%08d_img.png' % (args.exp_vis_dir, total_iter)
    ...
    target_path = '%s/%08d_gt_norm.png' % (args.exp_vis_dir, total_iter)
    ...
    for i in range(len(pred_norm_list)):
        ...
        target_path = '%s/%08d_pred_norm_%d.png' % (args.exp_vis_dir, total_iter, i)
        plt.imsave(target_path, pred_norm_rgb)

        pred_kappa = pred_kappa_list[i]
        pred_alpha = kappa_to_alpha(pred_kappa)
        target_path = '%s/%08d_pred_alpha_%d.png' % (args.exp_vis_dir, total_iter, i)
        ...
        E = E * gt_norm_mask
        target_path = '%s/%08d_pred_error_%d.png' % (args.exp_vis_dir, total_iter, i)
