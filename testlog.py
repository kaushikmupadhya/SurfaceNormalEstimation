...
def test(model, test_loader, device, results_dir,logger=None):
    alpha_max = 60
    kappa_max = 30
    ...
    with torch.no_grad():
        for data_dict in tqdm(test_loader):
            ...
            if logger is not None:
                logger['predicted_normal/'].append(File.as_image(pred_norm_rgb[0, :, :, :]))
                logger['predicted_kappa/'].append(File.as_image(pred_kappa[0, :, :, 0]))
                logger['predicted_alpha/'].append(File.as_image(pred_alpha[0, :, :, 0]))
...
if __name__ == '__main__':
    ...
    print("Trying to load the checkpoint")
    # load checkpoint
    ...
    # test the model
    print("Loading the Images directory")
    results_dir = args.imgs_dir + '/results'
    os.makedirs(results_dir, exist_ok=True)
    print("Testing the model with example images")
    ...
    test(model, test_loader, device, results_dir,logger=run)
