...
def train(model, args, device, logger):
    print("inside Train\n\n") #KSK
    if device is None:
    ...
    if args.dataset_name == 'nyu':
        ...
        train_loader = NyuLoader(args, 'train').data #Change here for training whole dataset, train small dataset #KSK
        test_loader = NyuLoader(args, 'test').data

    elif args.dataset_name == 'nyuv2':
        from data.dataloader_custom import MatDataLoader
        # path to the mat file dataset
        data_list = '/work/kmanhu2s/GeoNet/list/traindata_grid.txt'
        train_loader = MatDataLoader(file_path=data_list)
        test_loader = MatDataLoader(file_path=data_list)        
    else:
        ...
        for data_dict in t_loader:
            ...
            if args.rank == 0:
                ...
                if logger is not None:
                    # logger['loss/'].log(loss_)
                    logger["train/loss"].append(loss_)
                t_loader.refresh()
            ...
            if should_write and ((total_iter % args.validate_every) < args.batch_size_orig):
                ...
                validate(model, args, test_loader, device, total_iter, args.eval_acc_txt, logger=logger)
                model.train()
...
def validate(model, args, test_loader, device, total_iter, where_to_write, vis_dir=None,logger=None):
    print("\nInside validation") #KSK
    with torch.no_grad():
        ...
        print("\n total_normal_errors",total_normal_errors)
        total_normal_errors = total_normal_errors.data.cpu().numpy()
        print("\n total_normal_errors before passing to metrics",total_normal_errors)
        metrics = utils.compute_normal_errors(total_normal_errors)
        utils.log_normal_errors(metrics, where_to_write, first_line='total_iter: {}'.format(total_iter),logger=logger)
        return metrics
...
def main_worker(gpu,ngpus_per_node, args):
    ...
    if LOGGER:
        ...
        project="krishksk/uncertainty",
        api_token="eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vYXBwLm5lcHR1bmUuYWkiLCJhcGlfdXJsIjoiaHR0cHM6Ly9hcHAubmVwdHVuZS5haSIsImFwaV9rZXkiOiIyYTJhOWViNC1jOGUyLTRhYTUtYmQ4Ni1jMmM2M2M4NDBmYWYifQ==",
        tags = [str(args.exp_name)]) 
        # tags = [str(data_set_type),str(parameters['']),str(parameters['model_name']),"Robocup","Training"],
        # name= "Training" + "-" + str(data_set_type) + "-" + str(parameters['model_name']) + "-" + str(parameters['loss_function']),
    #######################

        hyper_parameters = {"batch_size": args.batch_size_orig}

        run['hyper_parameters/'] = hyper_parameters
    else:
        run = None
    ...
    train(model, args, device=args.gpu, logger= run)
...
if __name__ == '__main__':
    ...
    # loss function
    parser.add_argument('--loss_fn', default='UG_NLL_ours', type=str, help='{L1, L2, AL, NLL_vMF, NLL_ours, UG_NLL_vMF, UG_NLL_ours, WNLL, FWNL, WCLL, WP_NL}')
    ...
    print("ngpuspernode-",ngpus_per_node)
    args.num_workers = args.workers
    args.ngpus_per_node = ngpus_per_node

    args.batch_size_orig = args.batch_size

    #Neptune Logger krishksk #######################
    ...
    if args.distributed:
        ...
        print("inside args distributed\n\n!") #KSK
        mp.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, args))
    else:
        if ngpus_per_node == 1:
            print("inside args distributed ngpus per node--1\n\n!") #KSK
            args.gpu = 0
            print("This is with 1 gpu\n")
